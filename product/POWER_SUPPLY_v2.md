# Power Supply v2 - USB-C Power Delivery

Este documento descreve uma nova arquitetura de alimentação baseada em USB-C, inspirada no design do [Adafruit Matrix Portal S3](https://www.adafruit.com/product/5778) e no modelo de alimentação do Raspberry Pi 5. O objetivo é simplificar o PCB eliminando a necessidade de uma fonte DC dedicada.

---

## 1. Visão Geral

### 1.1 Filosofia de Design

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                      POWER SUPPLY v1 vs v2                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   v1 (Atual)                         v2 (Novo - USB-C)                      │
│   ══════════                         ════════════════                       │
│                                                                             │
│   ┌──────────┐                       ┌──────────┐                           │
│   │ FONTE DC │                       │ FONTE    │                           │
│   │ 5V/5A    │                       │ USB-C PD │                           │
│   │ 5.5x2.1mm│                       │ 45-65W   │                           │
│   └────┬─────┘                       └────┬─────┘                           │
│        │                                  │                                 │
│        ▼                                  ▼                                 │
│   ┌─────────┐                       ┌───────────┐                           │
│   │ DC JACK │                       │  USB-C    │                           │
│   │ on PCB  │                       │ Connector │                           │
│   └────┬────┘                       └─────┬─────┘                           │
│        │                                  │                                 │
│        │                            ┌─────┴─────┐                           │
│        │                            │PD Controller                          │
│        │                            │(opcional) │                           │
│        │                            └─────┬─────┘                           │
│        │                                  │                                 │
│        ▼                                  ▼                                 │
│   ┌─────────┐                       ┌───────────┐                           │
│   │ Painel  │                       │ Terminais │                           │
│   │  + PCB  │                       │ Parafuso  │                           │
│   └─────────┘                       └───────────┘                           │
│                                           │                                 │
│                                           ▼                                 │
│                                      ┌─────────┐                            │
│                                      │ Painel  │                            │
│                                      │  + PCB  │                            │
│                                      └─────────┘                            │
│                                                                             │
│   Vantagens v2:                                                             │
│   ✓ Fonte USB-C universal (telemóvel, laptop, etc.)                        │
│   ✓ Mais seguro (baixa tensão)                                             │
│   ✓ PCB mais simples e compacto                                            │
│   ✓ Cabo USB-C de qualidade é fácil de obter                               │
│   ✓ Terminais de parafuso para ligação robusta ao painel                   │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 1.2 Duas Estratégias Possíveis

| Estratégia | Descrição | Complexidade | Fiabilidade |
|------------|-----------|--------------|-------------|
| **A: 5V Direto** | USB-C 5V/5A direto (estilo Matrix Portal S3) | Baixa | Média |
| **B: Tensão Alta + Buck** | USB-C 9V/12V/20V + step-down para 5V | Média | Alta |

---

## 2. Estratégia A: 5V/5A Direto (Estilo Adafruit)

### 2.1 Conceito

Baseado no [Adafruit Matrix Portal S3](https://learn.adafruit.com/adafruit-matrixportal-s3/usb-power), a alimentação 5V entra diretamente pela porta USB-C e é distribuída para o painel através de terminais de parafuso.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ESTRATÉGIA A: 5V DIRETO                                  │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│                                                                             │
│   ┌────────────┐      ┌─────────────────────────────────────────────────┐  │
│   │            │      │                    PCB                          │  │
│   │  FONTE     │      │                                                 │  │
│   │  USB-C     │      │   ┌────────┐    ┌─────────┐    ┌────────────┐  │  │
│   │  5V/5A     ├──────┤──►│ USB-C  │───►│ FUSÍVEL │───►│ TERMINAIS  │  │  │
│   │  (25W)     │      │   │ Recept.│    │ + TVS   │    │ PARAFUSO   │  │  │
│   │            │      │   └────┬───┘    └────┬────┘    └─────┬──────┘  │  │
│   └────────────┘      │        │             │               │         │  │
│                       │        │             │               │         │  │
│                       │        ▼             │               ▼         │  │
│                       │   ┌────────┐         │         ┌───────────┐   │  │
│                       │   │ CH340C │         │         │  PAINEL   │   │  │
│                       │   │ USB-   │         │         │   P10     │   │  │
│                       │   │ UART   │         │         │ 32x16 RGB │   │  │
│                       │   └────┬───┘         │         └───────────┘   │  │
│                       │        │             │                         │  │
│                       │        │             ▼                         │  │
│                       │        │       ┌──────────┐                    │  │
│                       │        │       │ AP2112K  │                    │  │
│                       │        │       │  3.3V    │                    │  │
│                       │        │       └────┬─────┘                    │  │
│                       │        │            │                          │  │
│                       │        ▼            ▼                          │  │
│                       │   ┌─────────────────────┐                      │  │
│                       │   │      ESP32          │                      │  │
│                       │   │   (3.3V logic)      │                      │  │
│                       │   └─────────────────────┘                      │  │
│                       │                                                 │  │
│                       └─────────────────────────────────────────────────┘  │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 2.2 O Problema do 5V/5A no USB-C

O Raspberry Pi 5 [enfrenta o mesmo desafio](https://forums.raspberrypi.com/viewtopic.php?t=357395): **poucos carregadores USB-C suportam 5V/5A**.

```
USB Power Delivery (PD) - Perfis Comuns:
═══════════════════════════════════════════════════════════════

  Tensão │ Corrente Típica │ Potência │ Disponibilidade
  ───────┼─────────────────┼──────────┼──────────────────
   5V    │ 3A              │ 15W      │ ✓ MUITO COMUM
   5V    │ 5A              │ 25W      │ ✗ RARO (requer cabo 5A e-marked)
   9V    │ 3A              │ 27W      │ ✓ COMUM
   12V   │ 3A              │ 36W      │ ~ MÉDIO (PD 2.0, obsoleto em PD 3.1)
   15V   │ 3A              │ 45W      │ ✓ COMUM
   20V   │ 3A              │ 60W      │ ✓ MUITO COMUM
   20V   │ 5A              │ 100W     │ ✓ COMUM (laptops)

═══════════════════════════════════════════════════════════════

  ⚠️ PROBLEMA: O perfil 5V/5A (25W) é RARO!

  Motivos:
  1. Requer cabo USB-C com e-marker para >3A
  2. Fabricantes preferem subir tensão (mais eficiente)
  3. Especificação PD não obriga 5V/5A

  📌 Solução RPi5: Fonte oficial com chip PD proprietário
  📌 Alternativa: Override por software (usb_max_current_enable)

═══════════════════════════════════════════════════════════════
```

### 2.3 Implementação 5V Direto (Sem PD Controller)

Para fontes que fornecem 5V fixo (como carregadores básicos ou a fonte oficial RPi5):

```
Circuito Simplificado (Sem negociação PD):
═══════════════════════════════════════════════════════════════

                          USB-C Receptacle
                         ┌─────────────────┐
     VBUS (A4,A9,B4,B9) ─┤ VBUS            │
                         │                 │
                  GND ───┤ GND             │
                         │                 │
                  D+ ────┤ D+ (A6,B6)      │  → CH340C (dados)
                  D- ────┤ D- (A7,B7)      │  → CH340C (dados)
                         │                 │
           CC1 (A5) ─────┤ CC1   ┌─[5.1k]──┴── GND
           CC2 (B5) ─────┤ CC2   └─[5.1k]──┴── GND
                         └─────────────────┘
                                │
                                │ VBUS = 5V
                                ▼
    ┌───────────────────────────┼───────────────────────────────┐
    │                           │                               │
    │    ┌─────────────────┐    │    ┌─────────────────────┐   │
    │    │ F1: PTC 3A      │    │    │ D1: SMBJ5.0A TVS    │   │
    │    │ MF-MSMF300      │    │    │                     │   │
    │    └────────┬────────┘    │    └──────────┬──────────┘   │
    │             │             │               │               │
    │             └─────────────┼───────────────┘               │
    │                           │                               │
    │                           ▼                               │
    │                    ┌──────┴──────┐                        │
    │              C1    │   100µF     │   C2                   │
    │             470µF  │   Cerâmico  │  100nF                 │
    │              16V   │   10V       │   X7R                  │
    │                    └──────┬──────┘                        │
    │                           │                               │
    │                           ▼                               │
    │                      +5V_USB                              │
    │                           │                               │
    │         ┌─────────────────┼─────────────────┐             │
    │         │                 │                 │             │
    │         ▼                 ▼                 ▼             │
    │   ┌───────────┐    ┌───────────┐    ┌───────────┐        │
    │   │ AP2112K   │    │ 74AHCT245 │    │ TERMINAIS │        │
    │   │  3.3V     │    │ (5V VCC)  │    │ PARAFUSO  │        │
    │   └─────┬─────┘    └───────────┘    └─────┬─────┘        │
    │         │                                  │              │
    │         ▼                                  ▼              │
    │   ┌───────────┐                      ┌───────────┐        │
    │   │  ESP32    │                      │  PAINEL   │        │
    │   │  + RTC    │                      │   P10     │        │
    │   └───────────┘                      └───────────┘        │
    │                                                           │
    └───────────────────────────────────────────────────────────┘

Notas:
- Resistências 5.1kΩ nos CC1/CC2 anunciam "sink 5V/3A"
- Para 5A seria necessário PD controller com negociação
- Sem PD controller: limitado a 5V/3A (15W) máximo garantido

═══════════════════════════════════════════════════════════════
```

### 2.4 Análise de Consumo vs Disponibilidade

```
Análise de Potência - Painel P10 32x16:
═══════════════════════════════════════════════════════════════

  Componente        │ Tensão │ Corrente │ Notas
  ──────────────────┼────────┼──────────┼─────────────────────
  ESP32 + periféricos│ 3.3V   │ 200mA    │ WiFi ativo
  74AHCT245 (x2)    │ 5V     │ 20mA     │ Level shifters
  Painel P10 25%    │ 5V     │ 0.5A     │ Brilho reduzido
  Painel P10 50%    │ 5V     │ 1.0A     │ Uso típico
  Painel P10 75%    │ 5V     │ 1.5A     │ Brilho alto
  Painel P10 100%   │ 5V     │ 2.0A     │ Todos LEDs brancos
  Picos transitórios│ 5V     │ até 4A   │ Mudanças bruscas

═══════════════════════════════════════════════════════════════

  Cenários de Uso:
  ────────────────

  📱 5V/3A (15W) - Carregador comum:
     → ESP32: 0.2A + Painel 50%: 1.0A = 1.2A ✓
     → Margem confortável para uso típico
     → ⚠️ Pode ter brown-outs em picos

  🔌 5V/5A (25W) - Fonte RPi5 oficial:
     → Suporta todos os cenários
     → Margem para picos
     → ⚠️ Fonte específica necessária

═══════════════════════════════════════════════════════════════
```

---

## 3. Estratégia B: Tensão Alta + Buck Converter

### 3.1 Conceito

Usar uma tensão USB-C mais alta (9V, 15V ou 20V) e converter para 5V internamente. Isto resolve o problema de disponibilidade de fontes e reduz perdas no cabo.

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                    ESTRATÉGIA B: ALTA TENSÃO + BUCK                         │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌────────────┐                                                            │
│   │            │      ┌──────────────────────────────────────────────────┐ │
│   │  FONTE     │      │                      PCB                         │ │
│   │  USB-C PD  │      │                                                  │ │
│   │  45-65W    │      │  ┌────────┐  ┌────────┐  ┌────────┐  ┌────────┐ │ │
│   │            ├──────┤─►│ USB-C  │─►│  PD    │─►│ BUCK   │─►│TERMINAL│ │ │
│   │  9V/3A     │      │  │ Recept.│  │CONTROL.│  │5V/6A   │  │PARAFUSO│ │ │
│   │  15V/3A    │      │  └────┬───┘  └────┬───┘  └────┬───┘  └───┬────┘ │ │
│   │  20V/3A    │      │       │           │           │          │      │ │
│   │            │      │       │           │           │          │      │ │
│   └────────────┘      │       ▼           │           ▼          ▼      │ │
│                       │  ┌────────┐       │     ┌────────┐  ┌────────┐  │ │
│                       │  │ CH340C │       │     │ +5V    │  │ PAINEL │  │ │
│                       │  │ USB-   │       │     │ Rail   │  │  P10   │  │ │
│                       │  │ UART   │       │     └───┬────┘  └────────┘  │ │
│                       │  └────┬───┘       │         │                   │ │
│                       │       │           │         ▼                   │ │
│                       │       │           │   ┌──────────┐              │ │
│                       │       │           │   │ AP2112K  │              │ │
│                       │       │           │   │  3.3V    │              │ │
│                       │       │           │   └────┬─────┘              │ │
│                       │       │           │        │                    │ │
│                       │       ▼           │        ▼                    │ │
│                       │  ┌────────────────┴────────────────┐            │ │
│                       │  │            ESP32                │            │ │
│                       │  │         (3.3V logic)            │            │ │
│                       │  └─────────────────────────────────┘            │ │
│                       │                                                  │ │
│                       └──────────────────────────────────────────────────┘ │
│                                                                             │
│   Vantagens:                                                                │
│   ✓ Fontes 45-65W USB-C são MUITO comuns (laptops, tablets)                │
│   ✓ Menor corrente no cabo = menores perdas (I²R)                          │
│   ✓ Buck converter fornece 5V estável com alta eficiência (>90%)           │
│   ✓ Maior margem para picos de corrente                                    │
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘
```

### 3.2 Vantagens da Tensão Alta

```
Análise de Perdas no Cabo USB-C:
═══════════════════════════════════════════════════════════════

  Para fornecer 25W ao painel (5V × 5A):

  ┌────────────────┬───────────┬───────────┬───────────────────┐
  │ Tensão Entrada │ Corrente  │ Perda Cabo│ Notas             │
  │                │           │ (0.1Ω)    │                   │
  ├────────────────┼───────────┼───────────┼───────────────────┤
  │ 5V             │ 5.0A      │ 2.5W      │ Queda: 0.5V (10%)│
  │ 9V             │ 2.8A      │ 0.8W      │ Queda: 0.3V (3%) │
  │ 15V            │ 1.7A      │ 0.3W      │ Queda: 0.2V (1%) │
  │ 20V            │ 1.3A      │ 0.2W      │ Queda: 0.1V (1%) │
  └────────────────┴───────────┴───────────┴───────────────────┘

  Perda no cabo = I² × R_cabo

  💡 Usar 20V reduz perdas no cabo em 12x!

═══════════════════════════════════════════════════════════════

Disponibilidade de Fontes USB-C PD:
═══════════════════════════════════════════════════════════════

  ┌─────────────────────────────────────────────────────────────┐
  │                   DISPONIBILIDADE                           │
  │                                                             │
  │  5V/5A (25W)  ████░░░░░░░░░░░░░░░░░░░░  ~10% das fontes     │
  │                                                             │
  │  9V/3A (27W)  ████████████░░░░░░░░░░░░  ~50% das fontes     │
  │                                                             │
  │  15V/3A (45W) ████████████████░░░░░░░░  ~70% das fontes     │
  │                                                             │
  │  20V/3A (60W) ████████████████████░░░░  ~85% das fontes     │
  │                                                             │
  │  20V/5A (100W)████████████████████████  ~95% das fontes     │
  │                      (para laptops)                         │
  └─────────────────────────────────────────────────────────────┘

  ✓ Fontes de laptop (Dell, Lenovo, Apple) fornecem 20V/3A
  ✓ Carregadores GaN modernos suportam múltiplas tensões
  ✓ Praticamente qualquer fonte PD 45W+ funciona

═══════════════════════════════════════════════════════════════
```

### 3.3 Opções de PD Controller

```
Comparação de PD Controllers:
═══════════════════════════════════════════════════════════════

  ┌─────────────────┬─────────────────────┬────────────────────┐
  │                 │ CYPD3177            │ FUSB302B           │
  │                 │ (Infineon/Cypress)  │ (onsemi)           │
  ├─────────────────┼─────────────────────┼────────────────────┤
  │ Tipo            │ Sink-only           │ DRP/SRC/SNK        │
  │ Configuração    │ Hardware (pins)     │ Software (I2C)     │
  │ Tensões         │ 5V, 9V, 12V, 20V    │ Qualquer PD        │
  │ Correntes       │ 1A, 2A, 3A, 5A      │ Configurável       │
  │ Complexidade    │ Baixa (sem firmware)│ Alta (precisa MCU) │
  │ Package         │ 24-QFN              │ 14-WLCSP           │
  │ Preço           │ ~€1.50              │ ~€1.00             │
  │ LCSC            │ C2678129            │ C2828408           │
  └─────────────────┴─────────────────────┴────────────────────┘

  📌 Recomendação: CYPD3177 para simplicidade (não precisa firmware)

═══════════════════════════════════════════════════════════════

Alternativa Económica: IP2721
═══════════════════════════════════════════════════════════════

  O IP2721 é uma solução mais simples e barata:
  - Suporta PD 3.0
  - Configuração por resistor
  - Tensões: 5V, 9V, 12V, 15V, 20V
  - Package: DFN-10
  - Preço: ~€0.30

  ┌──────────────────────────────────────────────────────────┐
  │                   IP2721 Pinout                          │
  │                                                          │
  │   VBUS ──┬── [IP2721] ──┬── VOUT (à tensão selecionada) │
  │          │              │                                │
  │   CC1 ───┤              ├── SET (resistor seletor)      │
  │   CC2 ───┤              │                                │
  │          │              │                                │
  │   GND ───┴──────────────┴── GND                         │
  │                                                          │
  │   Resistor SET:                                          │
  │   - 0Ω    → 20V                                         │
  │   - 10kΩ  → 15V                                         │
  │   - 20kΩ  → 12V                                         │
  │   - 33kΩ  → 9V                                          │
  │   - 56kΩ  → 5V                                          │
  │                                                          │
  └──────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════
```

### 3.4 Buck Converter para 5V

```
Opções de Buck Converter:
═══════════════════════════════════════════════════════════════

  ┌─────────────────┬─────────────┬──────────┬────────────────┐
  │ Componente      │ Corrente    │ Vin      │ Eficiência     │
  ├─────────────────┼─────────────┼──────────┼────────────────┤
  │ MP2359          │ 1.2A        │ 4.5-24V  │ 92%            │
  │ MP1584          │ 3A          │ 4.5-28V  │ 92%            │
  │ TPS54531        │ 5A          │ 3.5-28V  │ 93%            │
  │ MP4560          │ 2A          │ 4.5-55V  │ 90%            │
  │ XL4015          │ 5A          │ 8-36V    │ 90%            │
  │ LM2596          │ 3A          │ 4.5-40V  │ 85%            │
  │ TPS5430         │ 3A          │ 5.5-36V  │ 90%            │
  │ SY8303          │ 3A          │ 4.5-30V  │ 93%            │
  └─────────────────┴─────────────┴──────────┴────────────────┘

  📌 Recomendação: MP1584EN (módulo) ou TPS54531 (IC)
     - MP1584EN: Módulo pré-montado, €0.80, fácil de usar
     - TPS54531: IC, €0.60, requer design de PCB

═══════════════════════════════════════════════════════════════

Circuito Típico MP1584EN:
═══════════════════════════════════════════════════════════════

                  Vin (9-20V do PD Controller)
                         │
                         ▼
    ┌────────────────────┼────────────────────┐
    │                    │                    │
    │   ┌────────────────┴────────────────┐   │
    │   │           MP1584EN              │   │
    │   │                                 │   │
    │   │  IN+      ┌────┐      OUT+      │   │
    │   │ ──────────┤    ├────────────────┼───┼──► +5V (6A max)
    │   │           │ FB │                │   │
    │   │  IN-      │    │      OUT-      │   │
    │   │ ──────────┤    ├────────────────┼───┼──► GND
    │   │           └────┘                │   │
    │   │                                 │   │
    │   │    [Ajuste: Potenciómetro       │   │
    │   │     ou resistor para 5.0V]      │   │
    │   │                                 │   │
    │   └─────────────────────────────────┘   │
    │                                         │
    │   Cin: 100µF 25V eletrolítico           │
    │   Cout: 220µF 10V eletrolítico          │
    │                                         │
    └─────────────────────────────────────────┘

  Especificações MP1584EN:
  - Entrada: 4.5V - 28V
  - Saída: 0.8V - 25V (ajustável)
  - Corrente: 3A contínua, 4A pico
  - Eficiência: ~92% @ 5V/3A
  - Frequência: 1.5MHz (indutor pequeno)

═══════════════════════════════════════════════════════════════
```

### 3.5 Diagrama Completo - Estratégia B

```
┌─────────────────────────────────────────────────────────────────────────────┐
│               ESTRATÉGIA B: CIRCUITO COMPLETO                               │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│                        USB-C Receptacle                                     │
│                       ┌─────────────────┐                                   │
│   VBUS ───────────────┤ VBUS (5-20V)    │                                   │
│                       │                 │                                   │
│   GND ────────────────┤ GND             │                                   │
│                       │                 │                                   │
│   D+ ─────────────────┤ D+              │───► CH340C (USB dados)            │
│   D- ─────────────────┤ D-              │───► CH340C (USB dados)            │
│                       │                 │                                   │
│   CC1 ────────────────┤ CC1             │───┐                               │
│   CC2 ────────────────┤ CC2             │───┤                               │
│                       └─────────────────┘   │                               │
│                              │              │                               │
│                              │              │                               │
│                              ▼              ▼                               │
│                       ┌─────────────────────────────┐                       │
│                       │        IP2721 / CYPD3177    │                       │
│                       │        PD Controller        │                       │
│                       │                             │                       │
│                       │  SET: [20kΩ] → 12V output   │                       │
│                       │       [0Ω]   → 20V output   │                       │
│                       │                             │                       │
│                       └──────────────┬──────────────┘                       │
│                                      │                                      │
│                              VOUT = 9V/12V/15V/20V                          │
│                                      │                                      │
│                       ┌──────────────┴──────────────┐                       │
│                       │                             │                       │
│                      ═╧═ C1                         │                       │
│                  100µF 35V                          │                       │
│                       │                             │                       │
│                       │                             ▼                       │
│                       │      ┌────────────────────────────────────┐         │
│                       │      │         BUCK CONVERTER             │         │
│                       │      │         (MP1584EN / TPS54531)      │         │
│                       │      │                                    │         │
│                       └─────►│  VIN                        VOUT   │──┬─► +5V│
│                              │                                    │  │      │
│                              │  GND ─────────────────────── GND   │  │      │
│                              │                                    │  │      │
│                              │  FB ←── [Voltage divider: 5.0V]   │  │      │
│                              │                                    │  │      │
│                              └────────────────────────────────────┘  │      │
│                                                                      │      │
│                    ┌─────────────────────────────────────────────────┘      │
│                    │                                                        │
│                    │         +5V RAIL (5A disponível)                       │
│                    │                                                        │
│         ┌──────────┼──────────────────┬──────────────────┐                  │
│         │          │                  │                  │                  │
│        ═╧═ C2      │                  │                  │                  │
│    470µF 10V       │                  │                  │                  │
│         │          │                  │                  │                  │
│         │          ▼                  ▼                  ▼                  │
│         │   ┌────────────┐    ┌────────────┐    ┌────────────────┐          │
│         │   │  AP2112K   │    │  74AHCT245 │    │   TERMINAIS    │          │
│         │   │   3.3V     │    │   (x2)     │    │   PARAFUSO     │          │
│         │   └──────┬─────┘    └────────────┘    └───────┬────────┘          │
│         │          │                                     │                  │
│         │          ▼                                     ▼                  │
│         │   ┌────────────┐                        ┌────────────┐            │
│         │   │   ESP32    │                        │   PAINEL   │            │
│         │   │   + RTC    │                        │    P10     │            │
│         │   └────────────┘                        └────────────┘            │
│         │                                                                   │
│         └───────────────────────────────────────────────────────────────────│
│                                                                             │
└─────────────────────────────────────────────────────────────────────────────┘

Componentes:
- U1: IP2721 ou CYPD3177 (PD Controller)
- U2: MP1584EN módulo ou TPS54531 (Buck 5V/5A)
- C1: 100µF 35V electrolítico (entrada buck)
- C2: 470µF 10V electrolítico (saída buck / bulk capacitor)
- C3: 100µF cerâmico 10V (saída buck, baixo ESR)

```

---

## 4. Workarounds para Limitações de Corrente

### 4.1 Software: Limite de Brilho

O [Adafruit Matrix Portal S3](https://learn.adafruit.com/adafruit-matrixportal-s3/usb-power) recomenda limitar o brilho por software quando alimentado por USB.

```cpp
// Limite de brilho por software (ESP32 + HUB75 library)
// ═══════════════════════════════════════════════════════════════

#include <ESP32-HUB75-MatrixPanel-I2S-DMA.h>

MatrixPanel_I2S_DMA *display = nullptr;

// Configuração com brilho limitado
void setupDisplay() {
    HUB75_I2S_CFG mxconfig(
        32,     // largura
        16,     // altura
        1       // chain length
    );

    display = new MatrixPanel_I2S_DMA(mxconfig);
    display->begin();

    // CRÍTICO: Limitar brilho para reduzir consumo
    // ─────────────────────────────────────────────
    //
    // Brilho 255 (100%): ~2A @ 5V (todos LEDs brancos)
    // Brilho 128 (50%):  ~1A @ 5V
    // Brilho 64 (25%):   ~0.5A @ 5V
    // Brilho 32 (12.5%): ~0.25A @ 5V
    //
    display->setBrightness8(64);  // 25% - Seguro para USB 5V/3A
}

// Brilho dinâmico baseado no conteúdo
// ───────────────────────────────────
// Se mostrar muitos pixels brancos, reduzir brilho
// Se mostrar poucos pixels, permitir brilho maior

uint8_t calculateSafeBrightness(uint16_t activePixels, uint16_t maxPixels) {
    // activePixels: número de pixels acesos
    // maxPixels: total de pixels (32 × 16 = 512)

    float fillRatio = (float)activePixels / maxPixels;

    // Fórmula: menos pixels = mais brilho permitido
    // Corrente máxima: 2.5A (para fonte 3A com margem)
    //
    // Se 100% pixels: brilho max 30%
    // Se 50% pixels:  brilho max 50%
    // Se 25% pixels:  brilho max 80%
    // Se 10% pixels:  brilho max 100%

    if (fillRatio > 0.8) return 64;      // 25%
    if (fillRatio > 0.5) return 96;      // 37%
    if (fillRatio > 0.25) return 128;    // 50%
    if (fillRatio > 0.1) return 192;     // 75%
    return 255;                           // 100%
}

// Perfis de consumo predefinidos
// ──────────────────────────────
enum PowerProfile {
    POWER_USB_3A,    // USB 5V/3A (15W) - brilho máx 25%
    POWER_USB_5A,    // USB 5V/5A (25W) - brilho máx 75%
    POWER_FULL       // Fonte dedicada  - brilho máx 100%
};

void setPowerProfile(PowerProfile profile) {
    switch (profile) {
        case POWER_USB_3A:
            display->setBrightness8(64);   // 25%
            break;
        case POWER_USB_5A:
            display->setBrightness8(192);  // 75%
            break;
        case POWER_FULL:
            display->setBrightness8(255);  // 100%
            break;
    }
}
```

### 4.2 Hardware: Bulk Capacitors

Condensadores de grande capacidade absorvem picos transitórios de corrente.

```
Dimensionamento de Bulk Capacitors:
═══════════════════════════════════════════════════════════════

  Cenário: Mudança de padrão no painel causa pico de 2A por 10ms

  Fórmula: C = I × Δt / ΔV

  - I = 2A (corrente de pico adicional)
  - Δt = 10ms (duração do pico)
  - ΔV = 0.3V (queda máxima aceitável: 5V → 4.7V)

  C = 2 × 0.01 / 0.3 = 67mF ≈ 68,000µF!  ← Impraticável

═══════════════════════════════════════════════════════════════

  Abordagem Prática:
  ──────────────────

  Usar combinação de condensadores com diferentes funções:

  ┌─────────────────────────────────────────────────────────────┐
  │                                                             │
  │  +5V ──┬── [470µF] ──┬── [100µF] ──┬── [10µF] ──┬── Load   │
  │        │  16V Elec.  │  Ceramic    │  Ceramic   │          │
  │        │  ESR ~0.1Ω  │  ESR ~5mΩ   │  ESR ~2mΩ  │          │
  │        │             │             │            │          │
  │       GND           GND           GND          GND         │
  │                                                             │
  │  470µF: Energia para picos longos (>1ms)                   │
  │  100µF cerâmico: Picos médios, baixo ESR                   │
  │  10µF cerâmico: Ruído alta frequência                      │
  │                                                             │
  └─────────────────────────────────────────────────────────────┘

  Alternativa: Super-capacitor 1F 5.5V
  ─────────────────────────────────────
  - Capacidade: 1F (1,000,000µF!)
  - ESR: ~0.1Ω
  - Preço: ~€2
  - Tamanho: Φ19mm × H18mm

  Pode manter 5V durante vários segundos de alta corrente.
  ⚠️ Cuidado: Inrush current elevado no arranque!

═══════════════════════════════════════════════════════════════

  Esquema com Super-capacitor:
  ════════════════════════════

  +5V_USB ───[F1 PTC]───┬───[D1 Schottky]───┬─── +5V_CAP
                        │                    │
                       ═╧═                  ═╧═
                   C1 100µF             SC1 1F/5.5V
                    16V                 (supercap)
                        │                    │
  GND ──────────────────┴────────────────────┴─── GND

  Notas:
  - D1 (1N5819): Previne descarga do supercap para USB
  - F1 (PTC 3A): Limita inrush current
  - SC1 carrega lentamente (~30s para carga completa)
  - Fornece energia extra durante picos

═══════════════════════════════════════════════════════════════
```

### 4.3 Hardware: Current Limiting com Soft-Start

```
Circuito de Inrush Current Limit:
═══════════════════════════════════════════════════════════════

  Problema: Ao ligar, condensadores grandes causam pico de corrente
            que pode disparar proteções da fonte USB.

  Solução: Resistor + MOSFET para soft-start

  ┌───────────────────────────────────────────────────────────────┐
  │                                                               │
  │  +5V_USB ──┬───[R1 10Ω]───┬───────────────────────► +5V_LOAD │
  │            │               │                                  │
  │            │        ┌──────┴──────┐                           │
  │            │        │   Q1 P-FET  │                           │
  │            │        │  (Si2301)   │                           │
  │            │        │   S     D   │                           │
  │            │        └──────┬──────┘                           │
  │            │               │                                  │
  │            │     ┌─────────┘                                  │
  │            │     │                                            │
  │            │     │   ┌─────────────┐                          │
  │            │     │   │   Timer     │                          │
  │            │     └───┤   555 ou    │                          │
  │            │         │   RC delay  │                          │
  │            │         └──────┬──────┘                          │
  │            │                │                                 │
  │  GND ──────┴────────────────┴────────────────────────► GND   │
  │                                                               │
  └───────────────────────────────────────────────────────────────┘

  Sequência:
  1. t=0: Fonte liga, corrente passa por R1 (limitada a 0.5A)
  2. t=100ms: Timer activa Q1, MOSFET bypassa R1
  3. t>100ms: Corrente fluí livremente pelo MOSFET

  Alternativa simples: NTC Thermistor
  ───────────────────────────────────

  +5V_USB ──[NTC 10Ω@25°C]───► +5V_LOAD

  - Frio: alta resistência, limita corrente
  - Aquece: baixa resistência, operação normal
  - Custo: €0.10
  - Desvantagem: não recupera instantaneamente após desligar

═══════════════════════════════════════════════════════════════
```

---

## 5. Terminais de Parafuso

### 5.1 Design Estilo Matrix Portal S3

O [Adafruit Matrix Portal S3](https://www.adafruit.com/product/5778) usa terminais de parafuso para ligar os 5V diretamente ao painel LED.

```
Layout dos Terminais:
═══════════════════════════════════════════════════════════════

  ┌─────────────────────────────────────────────────────────────┐
  │                                                             │
  │     SCREW TERMINAL BLOCK (2-pin, 5mm pitch)                │
  │                                                             │
  │     ┌─────────────────────────────────────────────────┐    │
  │     │                                                 │    │
  │     │    ┌─────────┐         ┌─────────┐             │    │
  │     │    │   (+)   │         │   (-)   │             │    │
  │     │    │  +5V    │         │   GND   │             │    │
  │     │    │         │         │         │             │    │
  │     │    │  ┌───┐  │         │  ┌───┐  │             │    │
  │     │    │  │ ◉ │  │         │  │ ◉ │  │ ← Parafusos│    │
  │     │    │  └───┘  │         │  └───┘  │             │    │
  │     │    │         │         │         │             │    │
  │     │    └─────────┘         └─────────┘             │    │
  │     │                                                 │    │
  │     └─────────────────────────────────────────────────┘    │
  │                                                             │
  │     Especificações:                                         │
  │     - Pitch: 5.0mm ou 5.08mm (0.2")                        │
  │     - Corrente: 10A+ rated                                  │
  │     - Fio: 12-22 AWG                                        │
  │     - Tipo: Spring-cage ou screw-clamp                      │
  │                                                             │
  └─────────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════

  Componentes Recomendados:
  ────────────────────────

  ┌─────────────────────────────────────────────────────────────┐
  │ Componente          │ Specs          │ Preço    │ LCSC     │
  ├─────────────────────────────────────────────────────────────┤
  │ KF301-5.0-2P        │ 5mm 2-pin 10A  │ €0.10    │ C430646  │
  │ Phoenix 1935161     │ 5.08mm 2-pin   │ €0.50    │ Premium  │
  │ Wago 236-402        │ Cage clamp     │ €0.60    │ Premium  │
  └─────────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════

  Ligação ao Painel P10:
  ──────────────────────

                    PCB
  ┌──────────────────────────────────────────────────────┐
  │                                                      │
  │  USB-C ──► [Circuito] ──► +5V ──┬─► [TERMINAL +]    │
  │                                 │                    │
  │                          GND ───┴─► [TERMINAL -]    │
  │                                                      │
  └──────────────────────────────────────────────────────┘
                                          │
                                          │ Fios 16-18 AWG
                                          │
                                          ▼
                               ┌─────────────────────┐
                               │    PAINEL P10       │
                               │                     │
                               │  VCC+ ──── VCC-     │
                               │                     │
                               └─────────────────────┘

  ⚠️ IMPORTANTE:
  - Usar fio de pelo menos 18 AWG (0.82mm²) para 3A+
  - Manter cabos curtos (<30cm) para minimizar perdas
  - Verificar polaridade antes de ligar!

═══════════════════════════════════════════════════════════════
```

### 5.2 Proteção contra Ligação Inversa nos Terminais

```
Proteção nos Terminais de Parafuso:
═══════════════════════════════════════════════════════════════

  ⚠️ Utilizadores podem ligar fios invertidos!

  Solução: P-MOSFET ou díodo Schottky

  +5V_INTERNO ──┬──[Q1 Si2301]──┬──[TERMINAL +]
                │    P-MOSFET   │
                │               │
               ─┴─ D1          ═╧═ C1
            Body diode         100µF
                │               │
  GND ──────────┴───────────────┴──[TERMINAL -]

  Funcionamento:
  - Polaridade correcta: MOSFET conduz (baixa perda)
  - Polaridade invertida: MOSFET bloqueia, body diode também
  - Protege todo o circuito interno

  Alternativa mais simples (com perda):
  ─────────────────────────────────────

  +5V_INTERNO ──[D1 SS54 Schottky]──[TERMINAL +]

  Perda: 0.3V × 3A = 0.9W (aceitável)

═══════════════════════════════════════════════════════════════
```

### 5.3 LEDs Indicadores de Power (Inspirado no MatrixPortal S3)

O [Adafruit Matrix Portal S3](https://github.com/adafruit/Adafruit-MatrixPortal-S3-PCB) inclui LEDs indicadores para diagnóstico visual do estado de alimentação. Esta é uma boa prática que facilita a resolução de problemas.

```
LEDs de Indicação de Power:
═══════════════════════════════════════════════════════════════

  ┌─────────────────────────────────────────────────────────────┐
  │                                                             │
  │  +5V ────[R1 1kΩ]────┬────[LED Verde]────► GND             │
  │                      │                                      │
  │                      └── Indica: 5V presente               │
  │                                                             │
  │  3.3V ───[R2 1kΩ]────┬────[LED Verde]────► GND             │
  │                      │                                      │
  │                      └── Indica: Regulador 3.3V OK         │
  │                                                             │
  └─────────────────────────────────────────────────────────────┘

  Especificações:
  ───────────────
  - LED: Verde 0603 ou 0805 (~2V forward, 5-10mA)
  - R1: 1kΩ para 5V → (5V - 2V) / 1kΩ = 3mA
  - R2: 1kΩ para 3.3V → (3.3V - 2V) / 1kΩ = 1.3mA

  Vantagens:
  ──────────
  ✓ Diagnóstico visual imediato
  ✓ Confirma que ambos os rails estão activos
  ✓ Baixo consumo (~4mA total)
  ✓ Útil para debugging durante desenvolvimento

═══════════════════════════════════════════════════════════════
```

---

## 6. Comparação das Estratégias

```
Tabela Comparativa:
═══════════════════════════════════════════════════════════════════════════════

  ┌─────────────────────┬─────────────────────────┬─────────────────────────┐
  │ Característica      │ ESTRATÉGIA A            │ ESTRATÉGIA B            │
  │                     │ (5V Direto)             │ (Alta Tensão + Buck)    │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Complexidade PCB    │ ⭐ BAIXA                │ ⭐⭐ MÉDIA               │
  │                     │ Apenas proteções        │ PD controller + buck    │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Custo componentes   │ €1.50                   │ €4.00                   │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Compatibilidade     │ ⚠️ LIMITADA            │ ⭐⭐⭐ EXCELENTE        │
  │ de fontes           │ Precisa 5V/5A ou 5V/3A  │ Qualquer PD 45W+        │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Corrente disponível │ 3A (típico)             │ 5-6A                    │
  │                     │ 5A (fonte especial)     │                         │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Perdas no cabo      │ ⚠️ ALTAS               │ ⭐ BAIXAS               │
  │                     │ I²R @ 5A = 2.5W         │ I²R @ 2A = 0.4W         │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Eficiência total    │ ~95%                    │ ~90% (inclui buck)      │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Brilho máximo       │ 25-50% (USB 3A)         │ 100%                    │
  │ recomendado         │ 75% (USB 5A)            │                         │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Estabilidade        │ ⚠️ Depende da fonte    │ ⭐⭐⭐ Excelente        │
  │                     │ e do cabo               │ Buck regula             │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Ruído/ripple        │ Depende da fonte        │ ⭐ Baixo (buck filtra)  │
  ├─────────────────────┼─────────────────────────┼─────────────────────────┤
  │ Segurança           │ ⭐⭐ Boa                │ ⭐⭐⭐ Melhor           │
  │                     │ Só 5V expostos          │ 5V isolados pelo buck   │
  └─────────────────────┴─────────────────────────┴─────────────────────────┘

═══════════════════════════════════════════════════════════════════════════════

  📌 RECOMENDAÇÃO:

  ┌─────────────────────────────────────────────────────────────────────────┐
  │                                                                         │
  │  Para PROTOTIPAGEM RÁPIDA:                                              │
  │  → Estratégia A (5V direto) com limite de brilho                       │
  │  → Usar fonte USB-C 5V/3A comum                                        │
  │  → Brilho máximo 40-50%                                                │
  │                                                                         │
  │  Para PRODUTO FINAL:                                                    │
  │  → Estratégia B (Alta tensão + buck)                                   │
  │  → Compatível com qualquer carregador laptop/tablet                    │
  │  → Brilho 100% sem restrições                                          │
  │  → Mais robusto e profissional                                         │
  │                                                                         │
  └─────────────────────────────────────────────────────────────────────────┘

═══════════════════════════════════════════════════════════════════════════════
```

---

## 7. Bill of Materials (BOM)

### 7.1 Estratégia A: 5V Direto

| Ref | Componente | Especificação | Package | Qty | Preço | LCSC |
|-----|------------|---------------|---------|-----|-------|------|
| J1 | USB-C Receptacle | 16-pin, USB 2.0 | SMD | 1 | €0.40 | C165948 |
| R1, R2 | Resistor 5.1kΩ | 1% 0.1W | 0402 | 2 | €0.01 | C25905 |
| F1 | PTC Fuse | 3A hold, 6A trip | 1812 | 1 | €0.15 | C369159 |
| D1 | TVS SMBJ5.0A | 5V 600W | SMB | 1 | €0.20 | C123799 |
| C1 | Capacitor | 470µF 16V electrolítico | Φ10mm | 1 | €0.15 | C134768 |
| C2 | Capacitor | 100µF 10V cerâmico | 1206 | 1 | €0.30 | C408880 |
| C3 | Capacitor | 100nF 16V X7R | 0402 | 1 | €0.01 | C307331 |
| J2 | Screw Terminal | 2-pin 5.0mm 10A | THT | 1 | €0.10 | C430646 |
| **Total** | | | | | **€1.32** | |

### 7.2 Estratégia B: Alta Tensão + Buck

| Ref | Componente | Especificação | Package | Qty | Preço | LCSC |
|-----|------------|---------------|---------|-----|-------|------|
| J1 | USB-C Receptacle | 16-pin, USB 2.0 | SMD | 1 | €0.40 | C165948 |
| U1 | IP2721 | PD Controller 5-20V | DFN-10 | 1 | €0.35 | C2889848 |
| R_SET | Resistor | Ver tabela tensão | 0402 | 1 | €0.01 | - |
| U2 | MP1584EN | Buck 3A module | Module | 1 | €0.80 | Módulo |
| ou | TPS54531 | Buck 5A IC | HTSSOP-14 | 1 | €0.60 | C130162 |
| L1 | Indutor | 22µH 5A (se TPS54531) | 10x10mm | 1 | €0.30 | C339828 |
| D2 | Schottky | SS54 5A (se TPS54531) | SMC | 1 | €0.10 | C123806 |
| F1 | PTC Fuse | 3A hold | 1812 | 1 | €0.15 | C369159 |
| D1 | TVS SMBJ24A | 24V (entrada) | SMB | 1 | €0.25 | C114152 |
| C1 | Capacitor | 100µF 35V electrolítico | Φ8mm | 1 | €0.15 | C249490 |
| C2 | Capacitor | 470µF 10V electrolítico | Φ10mm | 1 | €0.15 | C134768 |
| C3 | Capacitor | 100µF 10V cerâmico | 1206 | 1 | €0.30 | C408880 |
| C4 | Capacitor | 22µF 35V (entrada buck) | 1206 | 2 | €0.10 | C159770 |
| J2 | Screw Terminal | 2-pin 5.0mm 10A | THT | 1 | €0.10 | C430646 |
| **Total (módulo)** | | | | | **€2.95** | |
| **Total (IC)** | | | | | **€3.00** | |

### 7.3 Regulador 3.3V e LEDs (Comum a ambas estratégias)

| Ref | Componente | Especificação | Package | Qty | Preço | LCSC |
|-----|------------|---------------|---------|-----|-------|------|
| U_REG | AP2112K-3.3 | LDO 3.3V 600mA | SOT-23-5 | 1 | €0.15 | C51118 |
| C_IN | Capacitor | 10µF 10V X5R | 0603 | 1 | €0.02 | C19702 |
| C_OUT | Capacitor | 10µF 10V X5R | 0603 | 1 | €0.02 | C19702 |
| LED1 | LED Verde | 0805 ~2V 20mA | 0805 | 1 | €0.02 | C2297 |
| LED2 | LED Verde | 0805 ~2V 20mA | 0805 | 1 | €0.02 | C2297 |
| R_LED1 | Resistor | 1kΩ 1% | 0402 | 1 | €0.01 | C11702 |
| R_LED2 | Resistor | 1kΩ 1% | 0402 | 1 | €0.01 | C11702 |
| **Total** | | | | | **€0.25** | |

> **Nota:** O AP2112K-3.3 foi escolhido em vez do AMS1117 por ter:
> - Menor dropout (0.4V vs 1.0V)
> - Pino de enable para soft-start
> - Package mais compacto (SOT-23-5)
> - Usado no Adafruit MatrixPortal S3

### 7.4 Resistor SET para IP2721

| Tensão Desejada | Resistor SET |
|-----------------|--------------|
| 20V | 0Ω (curto) |
| 15V | 10kΩ |
| 12V | 20kΩ |
| 9V | 33kΩ |
| 5V | 56kΩ |

---

## 8. Esquema de Blocos Final

```
┌─────────────────────────────────────────────────────────────────────────────┐
│                 CIRCADIAN CLOCK v2.0 - USB-C POWER                          │
├─────────────────────────────────────────────────────────────────────────────┤
│                                                                             │
│   ┌─────────────┐      ┌────────────────────────────────────────────────┐  │
│   │   FONTE     │      │                    PCB                         │  │
│   │   USB-C     │      │                                                │  │
│   │   PD        │      │  ┌────────┐    ┌──────────┐    ┌────────────┐ │  │
│   │   45-65W    ├──────┼─►│ USB-C  │───►│ IP2721   │───►│ MP1584EN   │ │  │
│   │             │      │  │ Recept │    │ PD Ctrl  │    │ Buck 5V    │ │  │
│   └─────────────┘      │  └────┬───┘    │ (12V)    │    │            │ │  │
│                        │       │        └──────────┘    └──────┬─────┘ │  │
│                        │       │                               │       │  │
│                        │       │                               ▼       │  │
│                        │       │                        ┌──────────┐   │  │
│                        │       │                        │  +5V     │   │  │
│                        │       │                        │  RAIL    │   │  │
│                        │       │                        └────┬─────┘   │  │
│                        │       │                             │         │  │
│                        │       │    ┌────────────────────────┼───────┐ │  │
│                        │       │    │                        │       │ │  │
│                        │       ▼    ▼                        ▼       ▼ │  │
│                        │  ┌────────────┐  ┌────────────┐  ┌────────────┐│  │
│                        │  │ CH340C     │  │ AP2112K    │  │ SCREW     ││  │
│                        │  │ USB-UART   │  │ 3.3V       │  │ TERMINALS ││  │
│                        │  └─────┬──────┘  └──────┬─────┘  └─────┬─────┘│  │
│                        │        │                │              │      │  │
│                        │        │                ▼              │      │  │
│                        │        │     ┌──────────────────┐      │      │  │
│                        │        │     │      ESP32       │      │      │  │
│                        │        └────►│  (3.3V logic)    │      │      │  │
│                        │              │                  │      │      │  │
│                        │              │  ┌────────────┐  │      │      │  │
│                        │              │  │ 74AHCT245  │◄─┼──────┘      │  │
│                        │              │  │ 3.3V→5V    │  │  5V VCC     │  │
│                        │              │  └─────┬──────┘  │             │  │
│                        │              └────────┼─────────┘             │  │
│                        │                       │                       │  │
│                        │                       ▼                       │  │
│                        │              ┌────────────────┐               │  │
│                        │              │    HUB75       │◄──────────────┤  │
│                        │              │    HEADER      │  Painel 5V    │  │
│                        │              └────────────────┘               │  │
│                        │                                               │  │
│                        └───────────────────────────────────────────────┘  │
│                                                                           │
│                                            │                              │
│                                            ▼                              │
│                                    ┌───────────────┐                      │
│                                    │    PAINEL     │                      │
│                                    │     P10       │                      │
│                                    │   32x16 RGB   │                      │
│                                    └───────────────┘                      │
│                                                                           │
└───────────────────────────────────────────────────────────────────────────┘
```

---

## 9. Fontes USB-C Recomendadas

### 9.1 Para Estratégia A (5V/5A Direto)

| Fonte | Potência | Perfis | Preço | Notas |
|-------|----------|--------|-------|-------|
| Raspberry Pi 5 Official | 27W | 5V/5A | ~€15 | Específica para 5V/5A |
| Mean Well GS25U05 USB-C | 25W | 5V/5A | ~€12 | Industrial, fiável |

### 9.2 Para Estratégia B (Alta Tensão + Buck)

| Fonte | Potência | Perfis | Preço | Notas |
|-------|----------|--------|-------|-------|
| Anker 511 Nano 3 | 30W | 5V/3A, 9V/3A, 20V/1.5A | ~€18 | Compacto, GaN |
| Baseus 45W GaN | 45W | 5V/3A, 9V/3A, 15V/3A, 20V/2.25A | ~€15 | Bom custo-benefício |
| Ugreen 65W | 65W | Todos os perfis PD | ~€25 | 2 portas |
| Apple 30W USB-C | 30W | 5V/3A, 9V/3A, 15V/2A, 20V/1.5A | ~€35 | Premium, fiável |
| **Carregador laptop USB-C** | 45-100W | Todos | Incluído | Já disponível! |

> **💡 Dica:** Muitos utilizadores já têm carregadores USB-C de laptops que funcionam perfeitamente com a Estratégia B.

---

## 10. Firmware: Detecção Automática de Potência

```cpp
// firmware/power_management.h
// ═══════════════════════════════════════════════════════════════

#ifndef POWER_MANAGEMENT_H
#define POWER_MANAGEMENT_H

#include <Arduino.h>
#include <ESP32-HUB75-MatrixPanel-I2S-DMA.h>

// Pino ADC para medir tensão de entrada (com divisor)
#define VBUS_SENSE_PIN 36  // GPIO36 (ADC1_CH0)

// Divisor de tensão: 20V max → 3.3V
// R1 = 100k, R2 = 22k → Ratio = 22/(100+22) = 0.18
#define VOLTAGE_DIVIDER_RATIO 0.18

class PowerManager {
private:
    MatrixPanel_I2S_DMA *display;
    float inputVoltage;
    float maxBrightness;

public:
    PowerManager(MatrixPanel_I2S_DMA *dma_display) {
        display = dma_display;
        inputVoltage = 5.0;
        maxBrightness = 0.25;  // Conservador por defeito
    }

    // Medir tensão de entrada
    float measureInputVoltage() {
        int adcValue = analogRead(VBUS_SENSE_PIN);
        // ADC 12-bit: 0-4095 = 0-3.3V
        float adcVoltage = (adcValue / 4095.0) * 3.3;
        inputVoltage = adcVoltage / VOLTAGE_DIVIDER_RATIO;
        return inputVoltage;
    }

    // Determinar perfil de potência baseado na tensão
    void detectPowerProfile() {
        measureInputVoltage();

        Serial.printf("Tensão de entrada: %.1fV\n", inputVoltage);

        if (inputVoltage >= 18.0) {
            // USB PD 20V → Buck converter → 5V estável
            Serial.println("Perfil: PD 20V (100% brilho disponível)");
            maxBrightness = 1.0;  // 100%
        }
        else if (inputVoltage >= 14.0) {
            // USB PD 15V
            Serial.println("Perfil: PD 15V (100% brilho disponível)");
            maxBrightness = 1.0;
        }
        else if (inputVoltage >= 11.0) {
            // USB PD 12V
            Serial.println("Perfil: PD 12V (100% brilho disponível)");
            maxBrightness = 1.0;
        }
        else if (inputVoltage >= 8.0) {
            // USB PD 9V
            Serial.println("Perfil: PD 9V (100% brilho disponível)");
            maxBrightness = 1.0;
        }
        else if (inputVoltage >= 4.5) {
            // USB 5V directo (sem buck)
            // Assumir 3A máximo (fonte típica)
            Serial.println("Perfil: USB 5V direto (brilho limitado a 40%)");
            maxBrightness = 0.40;
        }
        else {
            Serial.println("AVISO: Tensão baixa!");
            maxBrightness = 0.20;
        }

        applyBrightness();
    }

    void applyBrightness() {
        uint8_t brightness = (uint8_t)(maxBrightness * 255);
        display->setBrightness8(brightness);
        Serial.printf("Brilho aplicado: %d%%\n", (int)(maxBrightness * 100));
    }

    // Ajustar brilho baseado no conteúdo
    void adjustForContent(uint16_t activePixels, uint16_t totalPixels) {
        float fillRatio = (float)activePixels / totalPixels;

        // Se muitos pixels activos, reduzir brilho para evitar overcurrent
        float contentFactor = 1.0;
        if (fillRatio > 0.7) contentFactor = 0.6;
        else if (fillRatio > 0.5) contentFactor = 0.8;

        uint8_t brightness = (uint8_t)(maxBrightness * contentFactor * 255);
        display->setBrightness8(brightness);
    }

    float getMaxBrightness() { return maxBrightness; }
    float getInputVoltage() { return inputVoltage; }
};

#endif
```

---

## 11. Referências

### Documentação Oficial
- [Adafruit Matrix Portal S3](https://www.adafruit.com/product/5778)
- [Adafruit Matrix Portal S3 - USB Power](https://learn.adafruit.com/adafruit-matrixportal-s3/usb-power)
- [Adafruit Matrix Portal S3 PCB (GitHub)](https://github.com/adafruit/Adafruit-MatrixPortal-S3-PCB)
- [Raspberry Pi 5 Power Requirements](https://circuitlabs.net/raspberry-pi-5-power-requirements-and-management/)

### Fóruns e Discussões
- [Raspberry Pi 5 5V/5A Power Issues](https://forums.raspberrypi.com/viewtopic.php?t=381080)
- [USB PD 5V/5A Availability Problem](https://forums.raspberrypi.com/viewtopic.php?t=357395)

### Componentes
- [CYPD3177 USB PD Sink Controller](https://hackaday.io/project/168762-usb-c-power-delivery-sink-cypd3177)
- [FUSB302B USB Type-C Controller](https://www.onsemi.com/products/interfaces/usb-type-c/fusb302b)
- [Cat-Sink USB-C PD Board](https://github.com/ElectronicCats/Cat-Sink)
- [AP2112K Datasheet (Diodes Inc)](https://www.diodes.com/assets/Datasheets/AP2112.pdf)

### Análise de Esquemas de Referência
- **MatrixPortal S3 Schematic Analysis (Dez 2024)**: Análise do esquema Eagle confirmou:
  - Design usa 5V direto sem PD controller (apenas resistências 5.1kΩ nos CC)
  - Regulador 3.3V: AP2112K-3.3 (600mA, low dropout)
  - Level shifters: 74AHCT245 (x2) para HUB75
  - LEDs indicadores de 5V e 3.3V para diagnóstico
  - Limitação: depende de fonte externa para correntes >3A

### USB Power Delivery
- [USB PD Specification Overview (Tom's Hardware)](https://www.tomshardware.com/peripherals/usb/the-usb-power-delivery-pd-specification-everything-you-need-to-know-about-usb-pd)
- [USB PD for General Purpose Use](https://www.electro-tech-online.com/threads/usb-c-pd-for-general-purpose-use.163644/)

---

*Documento criado: Dezembro 2024*
*Versão: 2.1 - USB-C Power Delivery*
*Atualizado: Dezembro 2024 - Análise do esquema MatrixPortal S3, substituição AMS1117→AP2112K, LEDs indicadores*
